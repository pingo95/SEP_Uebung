...
#include "../Header-Dateien/Qcustomplot.h"
...
*/
QCustomPlot::QCustomPlot(QWidget *parent, bool sorted) :
  QWidget(parent),
  xAxis(0),
  yAxis(0),
  xAxis2(0),
...
QCPGraph *QCustomPlot::addGraph(QCPAxis *keyAxis, QCPAxis *valueAxis)
{
  ...
  QCPGraph *newGraph = new QCPGraph(keyAxis, valueAxis,drawSorted);
...
*/
QCPGraph::QCPGraph(QCPAxis *keyAxis, QCPAxis *valueAxis, bool sorted) :
  QCPAbstractPlottable(keyAxis, valueAxis),drawSorted(sorted)
{
  mData = new QCPDataMap;
  // Towara:
  if(!drawSorted)
      mUnsortedData = new QCPUnsortedDataMap;
...
void QCPGraph::setData(const QVector<double> &key, const QVector<double> &value)
{
  ...
  if(!(this->drawSorted)){
      ...
      for (int i=0; i<n; ++i)
      {
        newData.key = key[i];
        newData.value = value[i];
        mUnsortedData->push_back(newData);
      }
  }
}
...
void QCPGraph::getPreparedData(QVector<QCPData> *lineData, QVector<QCPData> *scatterData) const
{
  ...
  if(lineData && !drawSorted){
      for(int i = 0; i < mUnsortedData->size(); i++){
          lineData->push_back((*mUnsortedData)[i]);
      }
  }
  if(scatterData && !drawSorted){
      for(int i = 0; i < mUnsortedData->size(); i++){
          scatterData->push_back((*mUnsortedData)[i]);
      }
  }
  if(!drawSorted)
     return;
